...
export async function POST(request: Request) {
  try {
    ...
    const validDomains = ['icloud.com', 'gatech.edu', 'gmail.com']; // Add your allowed domains
    const domain = email.split('@')[1];
    const institutionalDomains = ['gatech.edu', 'edu']; // Add other university domains

if (institutionalDomains.some(d => domain.includes(d))) {
  // Still try to send, but warn user
  console.warn(`Institutional email detected: ${domain}. Email may be blocked.`);
}
