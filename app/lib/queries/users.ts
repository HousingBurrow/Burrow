...
export async function updateUser({ id, email, firstName, lastName, gender, age }: updateUserProps): ActionResult<User> {
  try {
    const user = await prisma.user.update({ where: { id }, data: { email: email, first_name: firstName, last_name: lastName, gender: gender, age: age}});
    return {isError: false, data: user};
  } catch (e) {
